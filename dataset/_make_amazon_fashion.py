# python3 -m dataset.make_amazon_fashion
...
def filter_user_item_data():
    ...
    review_file = dsetroot / "Amazon" / "Amazon_Fashion" / "Amazon_Fashion.jsonl" #2500940
    item_file = dsetroot / "Amazon" / "Amazon_Fashion" / "meta_Amazon_Fashion.jsonl" #826109
    ...
    for line in iter_line(item_file, 826109):
    ...
    for line in iter_line(review_file, 2500940):
        rev = json.loads(line)
...
def format_feature_prompt(user_text, feature_name, spectrum):
    ...
    return f"""
You are evaluating a fashion goods reviewer's preferences.
...
def format_description_and_dynamic_prompt(user_text):
    return f"""
Based on the following fashion goods reviews:

{textwrap.shorten(user_text, width=6000)}

Generate:
1. A short summary (1–2 sentences) of the user's fashion goods preferences and habits.
2. A list of any notable preferences, special requests, or constraints — e.g. Stretchability, Fabric preference, Skin sensitivity, Color preferences, etc.
